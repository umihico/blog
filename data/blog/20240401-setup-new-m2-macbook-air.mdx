...
- Alfred
	...
	- 新しい側でも同期ファイルを生み出してからコピーしないと反映されないかも？
        - もしデータが飛んだら キーボードショートカット→Run Script `echo -n "$(date '+%Y-%m-%d')" | pbcopy`→Copy To Clipboard
- Obsidian
  - ホットキー
    - Move line up `Alt+Up`
    - Move line down `Alt+Down`
  - Daily Notes
    - ファイルロケーションを`[daily]/YYYY/MM/YYYY-MM-DD`に。
    - ホットキーCmd+Tを`Daily notes: Open today's daily note`に。New Tabと競合するので消す
  - Obsidian-Thino (旧Obsidian Memos)
    - ホットキーCmd+Mを`Thino: Focus on thino editor`に。New Tabと競合するので消す
- Rectangle（ダウンロードできなくなったspectacleから置換）
- [Voyagerのキーマップ](https://gist.github.com/umihico/b5a1de38d40b05b1af6360d3e514ed0e)。ショートカットの空きを調べたりするのに使う
- 12:40 Warp
- 12:45 .sshのコピー
...
- brew install gnupg
- brew install imagemagick (convertコマンドに必要）
